...
!interaction term
      if (r1==r2) then
      dm=dm*exp(-epa(r1,tau(l)))
      else
      dm=dm*exp(-tau(l-1)*(ljp(r1)+ljp(r2)))
      endif
      pw2(i,j,h)=dm
      enddo
      enddo
...
!-------------------------------------------------------------------
!function to calculate interaction term numerically
      real*8 function iljnum(x,y,t)
      implicit none
      real*8 x,y,t
      real*8 sig,eps,rc,apt,potail
      real*8 dt,tp,rt,a,pot,pterm,psum
      integer*4 nstep
      integer*4 i
      parameter(sig=2.556d0,eps=10.22d0,rc=8.d0)
